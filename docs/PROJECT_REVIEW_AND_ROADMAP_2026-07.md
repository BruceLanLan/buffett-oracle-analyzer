...
Phase D  可信度层深化：
         ...
         - regime 权重重验证 —— **已在 B1（v10.4.0, commit f3df8ad）完成**：
           ...
           的下一步，尚未做。
         - **2018-2020 COVID 窗口重验证 —— 已完成（2026-07-28，B3）**：
           `scripts/regime_weight_oos_2018_2020.py`（新脚本，复用
           `compute_cross_sectional_regime_ic`，在内存里临时恢复
           v10.6.0 禁用前的原始 multiplier 值，从不写回磁盘）用同一套
           37 票真实跑了 2018-01～2020-12 窗口。当年的假设**部分成立**：
           这次 BEAR_HIGH_VOL 桶真的覆盖到了 38 天、2018-02～2020-11
           跨度（包含 2018 年 2 月 Volmageddon 和 2020 年 3 月 COVID 崩盘
           两段真实市场事件，不再是 2022 窗口那种"完全覆盖不到"），样本量
           也比 2022 窗口的 9 天厚实得多。但结论和 2022 窗口一致：
           `regime_ic`（0.1371）比 `flat_ic`（0.1396）还**低**一点点，
           delta_mean = -0.0025，bootstrap CI = [-0.0123, 0.0010] 跨零，
           统计上分不清跟"没有效果"的区别。其余四个桶（SIDEWAYS/
           BULL_HIGH_VOL/BULL_LOW_VOL/BEAR_LOW_VOL）delta 同样很小
           （最大 +0.0153，n=32 天，未做显著性检验）。**这次是一个更强的
           负结论**——不再是"数据覆盖不到没法测"，而是"数据真覆盖到了、
           真测了、真没看出提升"，覆盖两个不同的真实熊市事件都是同一个
           结果。原来禁用的决定不需要重新考虑。附带发现一个真实的参数陷阱：
           `fetch_ticker_replay_records` 的 `period` 默认值 `"5y"` 是从
           **运行脚本当天**往回算的 yfinance 回看窗口，不是从请求的
           `end` 日期往回算——第一次跑 2018-2020 窗口时因为用了默认值，
           37 支股票全部静默返回 0 条记录，改成 `period="max"` 才修好，
           已经在 `fetch_ticker_replay_records` 的 docstring 里补了提醒。
