...
> 让18位传奇投资大师成为你的 AI Agent 伙伴，随时随地为你分析

---

## 一、产品形态全景

Augur 的18位投资人Agent可以独立存在，以多种形态嵌入你的工作流：
...
Augur 自带一个 Bloomberg 暗色风格的 FastAPI Web Dashboard，所有18位Agent自动加载：
...
| `/skill augur` → "所有大师对 NVDA 怎么看？" | 18位大师共识报告 |
...
支持个人微信(GeWeChat)、企业微信、Webhook三种模式。详见 [README](../README.md)。
...
└── serenity/SKILL.md            # Serenity (@aleabitoreddit) - AI/半导体供应链瓶颈
```
...
用户输入
  ...
  ├──→ Augur Dashboard (FastAPI) → src/augur/ → 18位 Agent → 共识 → 结果
...
**Q: 如何添加新的投资人？**
A: 两种方式：1) 在 `personas/custom/` 下写 YAML（无需代码）；2) 在 `src/augur/personas/` 下写 Python Agent（功能更强）。
