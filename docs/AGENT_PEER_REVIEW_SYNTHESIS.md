...
| P1-9 | Extract `dashboard/routes/workspace.py` (R1) | #4, #9 | ✅ Implemented (v10.16.13) — workspace routes + `/ws/workspace` + broadcast state moved to `dashboard/routes/workspace.py` (APIRouter); app.py −205 lines; 2136 tests pass, zero edits to existing tests. |
...
| P2-5 | `/ws/workspace` + workflow progress streaming | #4 | ✅ Implemented (v10.16.12) — `/ws/workspace` sends current state on connect then live `workspace_update` push on every `PUT /api/workspace` or profile switch via `asyncio.create_task` broadcast; `/ws/workflow` streams per-step `step_start`/`step_done`/`done` messages, uses `run_in_threadpool` to avoid blocking the event loop. |
| P2-6 | Lazy persona registration per profile | #9 | ✅ Implemented (v10.16.11) — `_BUILTIN_AGENTS` manifest dict + `importlib.import_module` dispatch in `_register_default_agents`; empty `enabled_personas` loads all 18 (backward compat). |
...
| P2-8 | `augur-terminal` meta-skill + Hermes committee yaml | #8 | ✅ Implemented (v10.16.12) — `skills/augur-terminal/` meta-skill (13 tools, 15 pages, 4 presets, standard workflow guide); `hermes-agents/committee.yaml` committee chair agent; `generate_skills.py` extended to generate augur-terminal. |
