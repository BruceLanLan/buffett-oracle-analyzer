...
| 10.16.12 | **P2-5 + P2-8**：`/ws/workspace`（实时广播工作区变更，`Set[WebSocket]` fan-out + `asyncio.create_task`）；`/ws/workflow`（流式步骤进度：每步发 `step_start`/`step_done`/`done`，阻塞调用用 `run_in_threadpool`）；`skills/augur-terminal/` meta-skill（15 页面、13 工具、4 预设、标准工作流）；`hermes-agents/committee.yaml`（委员会主席 Hermes agent）；`scripts/generate_skills.py` 扩展生成 augur-terminal 技能；新增 6 个 WebSocket 测试，共 26 个全绿 |
| 10.16.11 | **P2-1 + P2-6**：ConsensusEngine 抽取（`src/augur/consensus/engine.py`，从 `registry.py` 剥离 338 行共识逻辑，`DecisionCoordinator.get_consensus` 变成 7 行代理桩）；按 Profile 懒加载 persona（`AgentRegistry._register_default_agents` 读取 `get_enabled_personas()`，非空时只 import+实例化列出的 agent，空时加载全部 18 个，向后兼容）；新增 `_BUILTIN_AGENTS` manifest dict + 4 个 P2-6 专项测试（TestLazyPersonaRegistration）；hermes-agents/*.yaml 版本同步修复；v10.16.10 实际跳过（用于此次 P2-1 的 hermes yaml 同步），v10.16.11 是本次 P2-6 对应版本 |
