...
> 最后更新：2026-06-25，当前版本 **v10.16.8**（P2-3 窄版：regime 检测加 hysteresis + 历史时点取数，2108 tests passing；只解决了 synthesis 文档那条 gate 的一半——翻转抖动消除，手工权重数字本身是否有效仍未验证，留给 P2-4；v10.16.6 的投委会/深度报告 GIL 限制仍按用户决定保留为已知局限，详见下方叙事和 CHANGELOG）
...
- regime 检测已加 hysteresis + 历史时点取数（v10.16.8，P2-3 窄版）：`classify_regime` 纯函数核心，实盘/回测同一份逻辑；synthesis 文档那条"不要把共识结果当风险输入"gate 的翻转抖动那一半已解除，**手工权重数字本身是否有效那一半仍未验证**（P2-4 范畴）——详见下方叙事
- 测试基线：**2103 passed, 0 failed**（不含 5 个网络测试；含网络测试共 2108）
...
**下 session 待定：** 向用户确认 v10.16.5+v10.16.6+v10.16.7 修复是否真的改善了体验（仪表盘可点击、数据正常显示、历史页日历热力图能显示、优化器 Sharpe/权重计算正确、投委会/深度报告不再彻底卡死——但已知投委会/深度报告生成期间其他请求仍会有数秒延迟，这是已记录的限制，不需要每次都重新发现）；如果用户后续想彻底解决 GIL 拖慢问题，可从「给 18 位大师的 analyze() 做 profile 提速」或「换成进程池/多进程绕开 GIL」两个方向继续；v10.16.7 已经把 fetch()/fetchWithTimeout() 对应的接口逐个 curl 验证过一遍，覆盖了 history/optimizer/portfolio/watchlist/scanner/signals/settings/chat 几乎所有页面，剩下没碰过的是几个会产生副作用的接口（cron run-now、通知测试发送、真实 LLM 聊天）——如果以后要继续这条线，可以从这几个入手；**P2-3 已完成窄版（v10.16.8）**，P2-4（统一 OOS 校准管道，已获用户授权直接设计，且是把 `_REGIME_ADJUSTMENTS` 手工权重数字本身验证有效与否的唯一去处）顺理成章是下一个候选；P1-9（dashboard router 拆分）、P2-1/P2-2/P2-5/P2-6/P2-8 待选；用户分享了参考站点 chanlun.oldorange.club 作为产品形态参考，尚待讨论；v10.16.4 是本地 commit（`66f7424`）；v10.16.5+v10.16.6 已合并 commit 为 `1e57649`；v10.16.7 已 commit 为 `9508b03`；v10.16.8（P2-3 窄版）尚未 commit，停留在工作区。所有 commit 都**尚未推送到 origin**，需用户明确指示才能 push；公开的 `augur` 仓库不要碰，除非用户明确说要碰。
